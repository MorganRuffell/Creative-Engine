...
#define HALT( ... ) ERROR( __VA_ARGS__ ) __debugbreak();
    ...
    #define ASSERT( isFalse, ... ) \
        if (!(bool)(isFalse)) { \
            ...
            CUtility::PrintSubMessage("\'" #isFalse "\' is false"); \
            CUtility::PrintSubMessage(__VA_ARGS__); \
            CUtility::Print("\n"); \
            __debugbreak(); \
        }
    ...
    #define ASSERT_SUCCEEDED( hr, ... ) \
        if (FAILED(hr)) { \
            ...
            CUtility::PrintSubMessage("hr = 0x%08X", hr); \
            CUtility::PrintSubMessage(__VA_ARGS__); \
            CUtility::Print("\n"); \
            __debugbreak(); \
        }
    ...
    { \
        ...
        if ((bool)(isTrue) && !s_TriggeredWarning) { \
            ...
            CUtility::PrintSubMessage("\'" #isTrue "\' is true"); \
            CUtility::PrintSubMessage(__VA_ARGS__); \
            CUtility::Print("\n"); \
        } \
    }
