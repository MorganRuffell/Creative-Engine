...
namespace FXAA
{
    CRootSignature RootSig;
    CComputePSO Pass1HdrCS(L"FXAA: Pass 1 HDR CS");
    CComputePSO Pass1LdrCS(L"FXAA: Pass 1 LDR CS");
    CComputePSO ResolveWorkCS(L"FXAA: Resolve Work CS");
    CComputePSO Pass2HCS(L"FXAA: Pass 2 H CS");
    CComputePSO Pass2VCS(L"FXAA: Pass 2 V CS");
    CComputePSO Pass2HDebugCS(L"FXAA: Pass 2 H Debug CS");
    CComputePSO Pass2VDebugCS(L"FXAA: Pass 2 V Debug CS");
    CIndirectParamBuffer IndirectParameters;
    CByteAddressBuffer WorkCounters;

    CETBoolean Enable("Graphics/AA/FXAA/Enable", true);
    CETBoolean DebugDraw("Graphics/AA/FXAA/Debug", false);

    // With a properly encoded luma buffer, [0.25 = "low", 0.2 = "medium", 0.15 = "high", 0.1 = "ultra"]
    CETFloat ContrastThreshold("Graphics/AA/FXAA/Contrast Threshold", 0.175f, 0.05f, 0.5f, 0.025f);

    // Controls how much to blur isolated pixels that have little-to-no edge length.
    CETFloat SubpixelRemoval("Graphics/AA/FXAA/Subpixel Removal", 0.50f, 0.0f, 1.0f, 0.25f);
    ...
    CETBoolean ForceOffPreComputedLuma("Graphics/AA/FXAA/Always Recompute Log-Luma", false);
}
...
void FXAA::Render( CComputeContext& Context, bool bUsePreComputedLuma )
{
    if (ForceOffPreComputedLuma)
        bUsePreComputedLuma = false;

    CRGBBuffer& Target = g_bTypedUAVLoadSupport_R11G11B10_FLOAT ? g_SceneColorBuffer : g_PostEffectsBuffer;
