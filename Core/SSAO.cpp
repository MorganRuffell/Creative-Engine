...
namespace SSAO
{
    CETBoolean Enable("Graphics/SSAO/Enable", true);
    CETBoolean DebugDraw("Graphics/SSAO/Debug Draw", false);
    CETBoolean AsyncCompute("Graphics/SSAO/Async Compute", false);
    CETBoolean ComputeLinearZ("Graphics/SSAO/Always Linearize Z", true);
    ...
    CETStaticEnum g_QualityLevel("Graphics/SSAO/Quality Level", kSsaoQualityHigh, kNumSsaoQualitySettings, QualityLabels);
    ...
    CETFloat g_NoiseFilterTolerance("Graphics/SSAO/Noise Filter Threshold (log10)", -3.0f, -8.0f, 0.0f, 0.25f);
    CETFloat g_BlurTolerance("Graphics/SSAO/Blur Tolerance (log10)", -5.0f, -8.0f, -1.0f, 0.25f);
    CETFloat g_UpsampleTolerance("Graphics/SSAO/Upsample Tolerance (log10)", -7.0f, -12.0f, -1.0f, 0.5f);
    ...
    CETFloat RejectionFalloff("Graphics/SSAO/Rejection Falloff (rcp)", 2.5f, 1.0f, 10.0f, 0.5f);
    ...
    CETFloat Accentuation("Graphics/SSAO/Accentuation", 0.1f, 0.0f, 1.0f, 0.1f);

    CETInt HierarchyDepth("Graphics/SSAO/Hierarchy Depth", 3, 1, 4);

    void LinearizeZ(CComputeContext& Context, CGPUDepthBuffer& Depth, CRGBBuffer& LinearDepth, float zMagic);
}

namespace
{
    CRootSignature s_RootSignature;
    CComputePSO s_DepthPrepare1CS(L"SSAO: Depth Prepare 1 CS");
    CComputePSO s_DepthPrepare2CS(L"SSAO: Depth Prepare 2 CS");
    CComputePSO s_Render1CS(L"SSAO: Render 1 CS");
    CComputePSO s_Render2CS(L"SSAO: Render 2 CS");
    CComputePSO s_BlurUpsampleBlend[2] = { { L"SSAO: Blur Upsample Blend Low Q. CS" }, { L"SSAO: Blur Upsample Blend High Q. CS" } };	// Blend the upsampled result with the next higher resolution
    CComputePSO s_BlurUpsampleFinal[2] = { { L"SSAO: Blur Upsample Final Low Q. CS" }, { L"SSAO: Blur Upsample Final High Q. CS" } };	// Don't blend the result, just upsample it
    CComputePSO s_LinearizeDepthCS(L"SSAO: Linearize Depth CS");
    CComputePSO s_DebugSSAOCS(L"SSAO: Debug CS");
...
namespace SSAO
{
    void ComputeAO( CComputeContext& Context, CRGBBuffer& Destination, CRGBBuffer& DepthBuffer, const float TanHalfFovH )
    {
...
//#define SAMPLE_EXHAUSTIVELY
    ...
    void BlurAndUpsample( CComputeContext& Context,
        CRGBBuffer& Destination, CRGBBuffer& HiResDepth, CRGBBuffer& LoResDepth,
        CRGBBuffer* InterleavedAO, CRGBBuffer* HighQualityAO, CRGBBuffer* HiResAO
    )
    {
...
void SSAO::LinearizeZ(CComputeContext& Context, const CViewportCamera& camera, uint32_t FrameIndex)
{
    CGPUDepthBuffer& Depth = g_SceneDepthBuffer;
    CRGBBuffer& LinearDepth = g_LinearDepth[FrameIndex];
...
void SSAO::LinearizeZ(CComputeContext& Context, CGPUDepthBuffer& Depth, CRGBBuffer& LinearDepth, float zMagic)
{
...
void SSAO::Render( CGraphicsContext& GfxContext, const CViewportCamera& camera )
{
...
void SSAO::Render( CGraphicsContext& GfxContext, const float* ProjMat, float NearClipDist, float FarClipDist )
{
    ...
    if (!Enable)
    {
        CScopeTimingBlock _prof(L"Generate SSAO", GfxContext);
    ...
    CComputeContext& Context = AsyncCompute ? CComputeContext::Begin(L"Async SSAO", true) : GfxContext.GetCComputeContext();
    Context.SetRootSignature(s_RootSignature);

    { CScopeTimingBlock _prof(L"Decompress and downsample", Context);
    ...
    { CScopeTimingBlock _prof(L"Analyze depth volumes", Context);
    ...
    {  CScopeTimingBlock _prof(L"Blur and upsample", Context);

    // Phase 4:  Iteratively blur and upsample, combining each result

    CRGBBuffer* NextSRV = &g_AOMerged4;
    ...
    if (DebugDraw)
    {
        ...
        CComputeContext& CC = GfxContext.GetCComputeContext();
