...
uint32_t CRGBAColour::R11G11B10F(bool RoundToEven) const
{
#if 1
    ...
    R.f = CMath::Clamp(m_value.f[0], 0.0f, kMaxVal) * kF32toF16;
    G.f = CMath::Clamp(m_value.f[1], 0.0f, kMaxVal) * kF32toF16;
    B.f = CMath::Clamp(m_value.f[2], 0.0f, kMaxVal) * kF32toF16;
...
uint32_t CRGBAColour::R9G9B9E5() const
{
#if 1
    ...
    float r = CMath::Clamp(m_value.f[0], 0.0f, kMaxVal);
    float g = CMath::Clamp(m_value.f[1], 0.0f, kMaxVal);
    float b = CMath::Clamp(m_value.f[2], 0.0f, kMaxVal);

    // Compute the maximum channel, no less than 1.0*2^-15
    float MaxChannel = CMath::Max(CMath::Max(r, g), CMath::Max(b, kMinVal));
