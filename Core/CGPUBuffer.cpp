...
void CGPUBuffer::Create( const std::wstring& name, uint32_t NumElements, uint32_t ElementSize, const void* initialData )
{
    Destroy();
    ...
    if (initialData)
        CCommandContext::InitializeBuffer(*this, initialData, m_BufferSize);
...
void CGPUBuffer::Create( const std::wstring& name, uint32_t NumElements, uint32_t ElementSize, const CGraphicsUploadBuffer& srcData, uint32_t srcOffset )
{
    Destroy();
    ...
    CCommandContext::InitializeBuffer(*this, srcData, srcOffset);
...
void CGPUBuffer::Create(const std::wstring& name, uint32_t NumElements, uint32_t ElementSize, CGPURamAllocator& Allocator, const void* initialData)
{
    (void)Allocator;
...
D3D12_RESOURCE_DESC CGPUBuffer::DescribeBuffer(void)
{
...
void CByteAddressBuffer::CreateDerivedViews(void)
{
...
void CStructuredBuffer::CreateDerivedViews(void)
{
    D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
    SRVDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
...
void CTypeBasedBuffer::CreateDerivedViews(void)
{
    D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
    SRVDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
...
const D3D12_CPU_DESCRIPTOR_HANDLE& CStructuredBuffer::GetCounterSRV(CCommandContext& Context)
{
    Context.TransitionResource(m_CounterBuffer, D3D12_RESOURCE_STATE_GENERIC_READ);
    return m_CounterBuffer.GetSRV();
}

const D3D12_CPU_DESCRIPTOR_HANDLE& CStructuredBuffer::GetCounterUAV(CCommandContext& Context)
{
