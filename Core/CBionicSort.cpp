...
{
    CIndirectParamBuffer s_DispatchArgs;

    CRootSignature s_RootSignature;
    CComputePSO s_BitonicIndirectArgsCS(L"Bitonic Sort: Indirect Args CS");
    CComputePSO s_Bitonic32PreSortCS(L"Bitonic Sort: 32 Pre Sort CS");
    CComputePSO s_Bitonic32InnerSortCS(L"Bitonic Sort: 32 Inner Sort CS");
    CComputePSO s_Bitonic32OuterSortCS(L"Bitonic Sort: 32 Outer Sort CS");
    CComputePSO s_Bitonic64PreSortCS(L"Bitonic Sort: 64 Pre Sort CS");
    CComputePSO s_Bitonic64InnerSortCS(L"Bitonic Sort: 64 Inner Sort CS");
    CComputePSO s_Bitonic64OuterSortCS(L"Bitonic Sort: 64 Outer Sort CS");
...
void CAlgorithims::Initialize( void )
{	
...
void CAlgorithims::Shutdown( void )
{
    s_DispatchArgs.Destroy();
}

void CAlgorithims::Sort( 
    CComputeContext& Context,
    CGPUBuffer& KeyIndexList,
    CGPUBuffer& CounterBuffer,
...
)
{
    ...
    const uint32_t AlignedMaxNumElements = CMath::AlignPowerOfTwo(MaxNumElements);
    const uint32_t MaxIterations = CMath::Log2(std::max(2048u, AlignedMaxNumElements)) - 10;
...
{
    const T IndexMask = CMath::AlignPowerOfTwo(ListLength) - 1;
...
void TestBitonicSort(uint32_t ListSize, bool b64Bit, bool bAscending)
{
    ...
    const uint32_t IndexMask = CMath::AlignPowerOfTwo(ListSize) - 1;
    ...
    if (b64Bit)
    {
        ...
        for (uint32_t i = 0; i < ListSize; ++i)
            BufferPtr64[i] = ((uint64_t)(uint32_t)CMath::g_RNG.NextInt() << 32 | i);
    }
    else
    {
        uint32_t* BufferPtr32 = (uint32_t*)BufferPtr;
        for (uint32_t i = 0; i < ListSize; ++i)
            BufferPtr32[i] = (((uint32_t)CMath::g_RNG.NextInt() & ~IndexMask) | i);
    }

    // Upload list to GPU
    CByteAddressBuffer RandomListGpu;
    ...
    CByteAddressBuffer RandomListCount;
    ...
    CComputeContext& Ctx = CComputeContext::Begin(L"Bitonic Sort Test");
    CAlgorithims::Sort(Ctx, RandomListGpu, RandomListCount, 0, false, bAscending);
    Ctx.CopyBuffer(IndirectArgs, CAlgorithims::s_DispatchArgs);
