...
class CDXIndirectArgument
{
    friend class CCommandSignature;
public:
...
public:
    ...
    CCommandSignature( UINT NumParams = 0 ) : m_Finalized(FALSE), m_NumParameters(NumParams)
    {
        Reset(NumParams);
    }
    ...
    {
        if (NumParams > 0)
            m_ParamArray.reset(new CDXIndirectArgument[NumParams]);
        else
            m_ParamArray = nullptr;
    ...
    CDXIndirectArgument& operator[] ( size_t EntryIndex )
    {
        ASSERT(EntryIndex < m_NumParameters);
        return m_ParamArray.get()[EntryIndex];
    }

    const CDXIndirectArgument& operator[] ( size_t EntryIndex ) const
    {
        ASSERT(EntryIndex < m_NumParameters);
        return m_ParamArray.get()[EntryIndex];
    }

    void Finalize( const CRootSignature* RootSignature = nullptr );
...
private:
    std::unique_ptr<CDXIndirectArgument[]> m_ParamArray;
