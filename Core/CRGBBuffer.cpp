...
void CRGBBuffer::CreateDerivedViews(ID3D12Device8* Device, DXGI_FORMAT Format, uint32_t ArraySize, uint32_t NumMips)
{
    ...
    {
        m_RTVHandle = CGraphics::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        m_SRVHandle = CGraphics::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
    ...
    for (uint32_t i = 0; i < NumMips; ++i)
    {
        if (m_UAVHandle[i].ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
            m_UAVHandle[i] = CGraphics::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
void CRGBBuffer::CreateFromSwapChain( const std::wstring& Name, ID3D12Resource2* BaseResource )
{
    AssociateWithResource(CGraphics::g_Device, Name, BaseResource, D3D12_RESOURCE_STATE_PRESENT);
    ...
    m_RTVHandle = CGraphics::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    CGraphics::g_Device->CreateRenderTargetView(m_pResource.Get(), nullptr, m_RTVHandle);
}

void CRGBBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, 
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem)
{
    D3D12_RESOURCE_FLAGS Flags = CombineResourceFlags();
    D3D12_RESOURCE_DESC ResourceDesc = DescribeTex2D(Width, Height, 1, 0, Format, Flags);
    ...
    CreateTextureResource(CGraphics::g_Device, Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(CGraphics::g_Device, Format, 1);
}

void CRGBBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height,
    DXGI_FORMAT Format, CGPURamAllocator&)
{
    Create(Name, Width, Height, Format);
}

void CRGBBuffer::CreateArray( const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t ArrayCount,
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem )
{
    D3D12_RESOURCE_FLAGS Flags = CombineResourceFlags();
    ...
    CreateTextureResource(CGraphics::g_Device, Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(CGraphics::g_Device, Format, ArrayCount, 1);
}
...
void CRGBBuffer::GenerateMipMaps(CCommandContext& BaseContext)
{
    if (m_NumMipMaps == 0)
        return;
    ...
    Context.SetRootSignature(CGraphics::g_CommonRS);
    ...
    for (uint32_t TopMip = 0; TopMip < m_NumMipMaps; )
    {
        ...
        if (m_Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB)
            Context.SetPipelineState(CGraphics::g_GenerateMipsGammaPSO[NonPowerOfTwo]);
        else
            Context.SetPipelineState(CGraphics::g_GenerateMipsLinearPSO[NonPowerOfTwo]);

        // We can downsample up to four times
