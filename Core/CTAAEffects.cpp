...
using namespace CTAAEffects;

namespace CTAAEffects
{
    CETBoolean EnableTAA("Graphics/AA/TAA/Enable", false);
    CETFloat Sharpness("Graphics/AA/TAA/Sharpness", 0.5f, 0.0f, 1.0f, 0.25f);
    CETFloat TemporalMaxLerp("Graphics/AA/TAA/Blend Factor", 1.0f, 0.0f, 1.0f, 0.01f);
    ExponentialCETFloat TemporalSpeedLimit("Graphics/AA/TAA/Speed Limit", 64.0f, 1.0f, 1024.0f, 1.0f);
    CETBoolean TriggerReset("Graphics/AA/TAA/Reset", false);
    CETBoolean EnableCBR("Graphics/CBR/Enable", false);

    CComputePSO s_TemporalBlendCS(L"TAA: Temporal Blend CS");
    CComputePSO s_BoundNeighborhoodCS(L"TAA: Bound Neighborhood CS");
    CComputePSO s_SharpenTAACS(L"TAA: Sharpen TAA CS");
    CComputePSO s_ResolveTAACS(L"TAA: Resolve TAA CS");
    ...
    void ApplyTemporalAA(_In_ CComputeContext& Context);
    void SharpenImage(_In_ CComputeContext& Context, _In_ CRGBBuffer& TemporalColor);
}

void CTAAEffects::Initialize( void )
{
...
void CTAAEffects::Shutdown( void )
{
}

void CTAAEffects::Update( uint64_t FrameIndex )
{
...
uint32_t CTAAEffects::GetFrameIndexMod2( void )
{
    return s_FrameIndexMod2;
}

void CTAAEffects::GetJitterOffset( float& JitterX, float& JitterY )
{
    JitterX = s_JitterX;
    JitterY = s_JitterY;
}

void CTAAEffects::ClearHistory( CCommandContext& Context )
{
    CGraphicsContext& gfxContext = Context.GetGraphicsContext();
...
void CTAAEffects::ResolveImage( _In_ CCommandContext& BaseContext )
{
...
void CTAAEffects::ApplyTemporalAA(_In_ CComputeContext& Context)
{
...
void CTAAEffects::SharpenImage(_In_ CComputeContext& Context, _In_ CRGBBuffer& TemporalColor)
{
