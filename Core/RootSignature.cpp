...
void CRootSignature::DestroyAll(void)
{
...
void CRootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
	...
	size_t HashCode = CUtility::HashState(&RootDesc.Flags);
	HashCode = CUtility::HashState(RootDesc.pStaticSamplers, m_NumSamplers, HashCode);

	bool firstCompile = false;
	ID3D12RootSignature** RSRef = nullptr;


	std::thread FinalRootThread = std::thread([&] {
	...
	for (UINT Param = 0; Param < m_NumParameters; ++Param)
	{
		...
		if (RootParam.ParameterType == D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE)
		{
			...
			HashCode = CUtility::HashState(RootParam.DescriptorTable.pDescriptorRanges,
			...
			HashCode = CUtility::HashState(&RootParam, 1, HashCode);
	}

	FinalRootThread.join();
