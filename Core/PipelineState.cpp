...
CGraphicsPSO::CGraphicsPSO(const wchar_t* Name)
    : CBasePipelineStateObject(Name)
{
...
void CGraphicsPSO::SetDepthTargetFormat(DXGI_FORMAT DSVFormat, UINT MsaaCount, UINT MsaaQuality )
{
...
void CGraphicsPSO::Finalize()
{
    ...
    size_t HashCode = CUtility::HashState(&m_PSODesc);
    HashCode = CUtility::HashState(m_InputLayouts.get(), m_PSODesc.InputLayout.NumElements, HashCode);
...
CComputePSO::CComputePSO(const wchar_t* Name)
    : CBasePipelineStateObject(Name)
{
    ZeroMemory(&m_PSODesc, sizeof(m_PSODesc));
    m_PSODesc.NodeMask = 1;
