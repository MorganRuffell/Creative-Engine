...
//We can dictate what format we want our buffer to be. 
DXGI_FORMAT CPixelBuffer::GetBaseFormat( DXGI_FORMAT defaultFormat )
{
...
DXGI_FORMAT CPixelBuffer::GetUAVFormat( DXGI_FORMAT defaultFormat )
{
    switch (defaultFormat)
    {
...
DXGI_FORMAT CPixelBuffer::GetDSVFormat( DXGI_FORMAT defaultFormat )
{
    switch (defaultFormat)
    {
    // 32-bit Z w/ Stencil
...
DXGI_FORMAT CPixelBuffer::GetDepthFormat( DXGI_FORMAT defaultFormat )
{
    switch (defaultFormat)
    {
    // 32-bit Z w/ Stencil
...
DXGI_FORMAT CPixelBuffer::GetStencilFormat( DXGI_FORMAT defaultFormat )
{
    switch (defaultFormat)
    {
    // 32-bit Z w/ Stencil
...
//--------------------------------------------------------------------------------------
size_t CPixelBuffer::BytesPerPixel( DXGI_FORMAT Format )
{
    switch( Format )
    {
...
void CPixelBuffer::AssociateWithResource(_Inout_ ID3D12Device8* Device, const std::wstring& Name, _Inout_ ID3D12Resource2* Resource, D3D12_RESOURCE_STATES CurrentState )
{
...
D3D12_RESOURCE_DESC CPixelBuffer::DescribeTex2D( uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize,
...
void CPixelBuffer::CreateTextureResource( ID3D12Device8* Device, const std::wstring& Name,
...
void CPixelBuffer::CreateTextureResource( ID3D12Device8* Device, const std::wstring& Name,
    const D3D12_RESOURCE_DESC& ResourceDesc, D3D12_CLEAR_VALUE ClearValue, CGPURamAllocator& Allocator )
{
    (Allocator);
...
void CPixelBuffer::ExportToFile( const std::wstring& FilePath )
{
    ...
    CCommandContext& Context = CCommandContext::Begin(L"Copy texture to memory");
    uint32_t RowPitch = Context.ReadbackTexture(TempBuffer, *this);
    Context.Finish(true);

    // Retrieve a  pointer to the buffer memory.  Map the whole range for reading.
