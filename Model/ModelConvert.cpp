...
#include "Import/External/ofbx.cpp"
...
static inline CVector3 SafeNormalize(CVector3 x)
{
    float lenSq = LengthSquare(x);
    return lenSq < 1e-10f ? CVector3(kXUnitVector) : x * RecipSqrt(lenSq);
}

void CBaseRenderer::CompileMesh(
    std::vector<CStaticMesh*>& meshList,
    std::vector<cs_byte>& bufferMemory,
    glTF::CBaseMesh& srcMesh,
    uint32_t matrixIdx,
    const CMatrix4& localToObject,
    CSphereCollider& boundingSphere,
    CAxisAlignedBox& boundingBox
    )
{
    ...
    CSphereCollider sphereOS(kZero);
    CAxisAlignedBox bboxOS(kZero);
    ...
    for (auto& prim : primitives)
    {
        ...
        totalIndexSize += CMath::AlignUp(prim.IB->size(), 4);
    }

    uint32_t totalBufferSize = (uint32_t)(totalVertexSize + totalDepthVertexSize + totalIndexSize);

    CUtility::CSmartByteVector stagingBuffer;
    stagingBuffer.reset(new std::vector<cs_byte>(totalBufferSize));
    ...
    for (auto& iter : renderMeshes)
    {
        ...
        CStaticMesh* mesh = (CStaticMesh*)malloc(sizeof(CStaticMesh) + sizeof(CStaticMesh::Draw) * (numDraws - 1));
        size_t vbSize = 0;
        size_t vbDepthSize = 0;
        size_t ibSize = 0;

        // Compute local space bounding sphere for all submeshes
        CSphereCollider collectiveSphere(kZero);
        ...
        for (auto& draw : iter.second)
        {
            CStaticMesh::Draw& d = mesh->draw[drawIdx++];
        ...
        curIBOffset += (uint32_t)CMath::AlignUp(ibSize, 4);
        curIndexOffset = CMath::AlignUp(curIndexOffset, 4);
...
static uint32_t WalkGraph(
    std::vector<CGraphNode>& sceneGraph,
    CSphereCollider& modelBSphere,
    CAxisAlignedBox& modelBBox,
    std::vector<CStaticMesh*>& meshList,
    std::vector<cs_byte>& bufferMemory,
    const std::vector<glTF::Node*>& siblings,
    uint32_t curPos,
    const CMatrix4& xform
    )
{
    ...
    for (size_t i = 0; i < numSiblings; ++i)
    {
        ...
        CGraphNode& thisGraphNode = sceneGraph[curPos];
        ...
        {
            thisGraphNode.xform = CMatrix4(
                CMatrix3(thisGraphNode.rotation) * CMatrix3::MakeScale(thisGraphNode.scale),
                CVector3(*(const XMFLOAT3*)curNode->translation)
            );
        }

        const CMatrix4 LocalXform = xform * thisGraphNode.xform;

        if (!curNode->pointsToCamera && curNode->mesh != nullptr)
        {
            CSphereCollider sphereOS;
            CAxisAlignedBox boxOS;
...
{
    CompileTextureOnDemand(basePath + CUtility::UTF8ToWideString(fileName), flags);
}
...
void BuildMaterials(ModelData& model, const glTF::Asset& asset)
{
    ...
    {
        const glTF::CSimpleMaterial& srcMat = asset.m_materials[i];
    ...
    for (auto name : model.m_TextureNames)
    {
        ...
        if (iter != textureOptions.end())
        {
            ...
            CompileTextureOnDemand(asset.m_basePath + CUtility::UTF8ToWideString(iter->first), iter->second);
        }
        else
...
void BuildAnimations(ModelData& model, const glTF::Asset& asset)
{
    ...
    {
        CAnimationSet& animSet = model.m_Animations[animIdx++];
        ...
        for (size_t i = 0; i < animSet.numCurves; ++i)
        {
            ...
            curve.keyFrameFormat = std::min<uint32_t>(sampler.m_output->componentType, CAnimationCurve::kFloat);
...
void BuildSkins(ModelData& model, const glTF::Asset& asset)
{
    ...
    for (const glTF::Skin& skin : asset.m_skins)
    {
        ...
        // Append IBMs
        CMatrix4* IBMstart = (CMatrix4*)skin.inverseBindMatrices->dataPtr;
        CMatrix4* IBMend = IBMstart + skin.inverseBindMatrices->count;
    ...
    for (CSkinnedMesh* mesh : model.m_SkinnedMeshes)
    {
        if (mesh->numJoints != 0)
        {
...
bool CBaseRenderer::BuildModel(ModelData& model, const glTF::Asset& asset, int sceneIdx)
{
    ...
    std::vector<cs_byte>& bufferMemory = model.m_GeometryData;

    model.m_BoundingSphere = CSphereCollider(kZero);
    model.m_BoundingBox = CAxisAlignedBox(kZero);
    uint32_t numNodes = WalkGraph(model.m_SceneGraph, model.m_BoundingSphere, model.m_BoundingBox, model.m_Meshes, bufferMemory, scene->nodes, 0, CMatrix4(kIdentity));
...
bool CBaseRenderer::SaveModel(const std::wstring& filePath, const ModelData& data)
{
    ...
    for (const CStaticMesh* mesh : data.m_Meshes)
        header.meshDataSize += (uint32_t)sizeof(CStaticMesh) + (mesh->numDraws - 1) * (uint32_t)sizeof(CStaticMesh::Draw);

    for (const CSkinnedMesh* mesh : data.m_SkinnedMeshes)
        header.meshDataSize += (uint32_t)sizeof(CSkinnedMesh) + (mesh->numDraws - 1) * (uint32_t)sizeof(CSkinnedMesh::Draw);
    ...
    outFile.write((char*)&header, sizeof(CFileHeader));
    outFile.write((char*)data.m_GeometryData.data(), header.geometrySize);
    outFile.write((char*)data.m_SceneGraph.data(), header.numNodes * sizeof(CGraphNode));

    for (const CStaticMesh* mesh : data.m_Meshes)
        outFile.write((char*)mesh, sizeof(CStaticMesh) + (mesh->numDraws - 1) * sizeof(CStaticMesh::Draw));

    for (const CSkinnedMesh* mesh : data.m_SkinnedMeshes)
        outFile.write((char*)mesh, sizeof(CSkinnedMesh) + (mesh->numDraws - 1) * sizeof(CSkinnedMesh::Draw));
    ...
    if (header.numAnimations > 0)
    {
        ...
        outFile.write((char*)data.m_AnimationCurves.data(), header.numAnimationCurves * sizeof(CAnimationCurve));
        outFile.write((char*)data.m_Animations.data(), header.numAnimations * sizeof(CAnimationSet));
    }
    else
    {
        ASSERT(header.keyFrameDataSize == 0 && header.numAnimationCurves == 0);
    ...
    if (header.numJoints)
    {
        ...
        outFile.write((char*)data.m_JointIBMs.data(), header.numJoints * sizeof(CMatrix4));
    }
