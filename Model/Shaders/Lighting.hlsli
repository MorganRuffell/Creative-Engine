...
ConstantBuffer<PSConstants> PixelShaderConst: register(b0);
...
// FSchlicks albedo -- Modulates light
...
float GetDirectionalShadow( float3 ShadowCoord, Texture2D<float> texShadow )
{
    ...
    float d1 = Dilation * PixelShaderConst.ShadowTexelSize.x * 0.125;
    float d2 = Dilation * PixelShaderConst.ShadowTexelSize.x * 0.875;
    float d3 = Dilation * PixelShaderConst.ShadowTexelSize.x * 0.625;
    float d4 = Dilation * PixelShaderConst.ShadowTexelSize.x * 0.375;
    float result = (
...
{
    uint2 tilePos = GetTilePos(pixelPos, PixelShaderConst.InvTileDim.xy);
    uint tileIndex = GetTileIndex(tilePos, PixelShaderConst.TileCount.x);
