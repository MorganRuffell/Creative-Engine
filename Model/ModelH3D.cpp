...
void CMesh::Clear()
{
    ...
    m_Header.boundingBox = CAxisAlignedBox();
}

// assuming at least 3 floats for position
void CMesh::ComputeMeshBoundingBox(uint32_t meshIndex, CAxisAlignedBox& bbox) const
{
    const CBaseMesh *mesh = m_pMesh + meshIndex;

    bbox = CAxisAlignedBox();
...
void CMesh::ComputeGlobalBoundingBox(CAxisAlignedBox &bbox) const
{
    bbox = CAxisAlignedBox();
...
void CMesh::ComputeAllBoundingBoxes()
{
    for (uint32_t meshIndex = 0; meshIndex < m_Header.meshCount; meshIndex++)
    {
        CBaseMesh *mesh = m_pMesh + meshIndex;
        ComputeMeshBoundingBox(meshIndex, mesh->boundingBox);
    }
    ComputeGlobalBoundingBox(m_Header.boundingBox);
}

bool CMesh::LoadH3D(const wstring& filename)
{
    std::ifstream file(filename, ios::in | ios::binary);

    file.read((char*)&m_Header, sizeof(CrytekMeshHeader));
    if (!file)
        return false;

    m_pMesh = new CBaseMesh[m_Header.meshCount];
    m_pMaterial = new CSimpleMaterial[m_Header.materialCount];

    if (m_Header.meshCount > 0)
    {
        file.read((char*)m_pMesh, sizeof(CBaseMesh) * m_Header.meshCount);
        if (!file)
            return false;
    }
    if (m_Header.materialCount > 0)
    {
        file.read((char*)m_pMaterial, sizeof(CSimpleMaterial) * m_Header.materialCount);
        if (!file)
            return false;
    }
    ...
    for (uint32_t meshIndex = 0; meshIndex < m_Header.meshCount; ++meshIndex)
    {
        ...
        ASSERT(mesh.attrib[0].components == 3 && mesh.attrib[0].format == CMesh::attrib_format_float); // position
        ASSERT(mesh.attrib[1].components == 2 && mesh.attrib[1].format == CMesh::attrib_format_float); // texcoord0
        ASSERT(mesh.attrib[2].components == 3 && mesh.attrib[2].format == CMesh::attrib_format_float); // normal
        ASSERT(mesh.attrib[3].components == 3 && mesh.attrib[3].format == CMesh::attrib_format_float); // tangent
        ASSERT(mesh.attrib[4].components == 3 && mesh.attrib[4].format == CMesh::attrib_format_float); // bitangent
    ...
    CGraphicsUploadBuffer geomBuffer;
    ...
    LoadTextures(CUtility::GetBasePath(filename));

    return true;
}

void CMesh::CreateVertexBufferSRV(D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle) const
{
    ...
    CGraphics::g_Device->CreateShaderResourceView(const_cast<ID3D12Resource2*>(m_GeometryBuffer.GetResource()), &srvDesc, cpuHandle);
}

void CMesh::CreateIndexBufferSRV(D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle) const
{
    D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
...
bool CMesh::SaveH3D(const wstring& filename) const
{
    std::ofstream file(filename, ios::out | ios::binary);
    if (!file)
        return false;

    if (!file.write((char*)&m_Header, sizeof(CrytekMeshHeader)))
        return false;

    if (m_Header.meshCount > 0 && !file.write((const char*)m_pMesh, sizeof(CBaseMesh) * m_Header.meshCount))
        return false;

    if (m_Header.materialCount > 0 && !file.write((const char*)m_pMaterial, sizeof(CSimpleMaterial) * m_Header.materialCount))
        return false;
...
{
    return CUtility::RemoveExtension(CUtility::UTF8ToWideString(std::string(filename)));
}

void CMesh::LoadTextures(const std::wstring& basePath)
{
    using namespace CGraphics;
    using namespace CTextureManager;

    m_TextureReferences.resize(m_Header.materialCount * 3);
    m_SRVs = CBaseRenderer::s_TextureHeap.Alloc(m_Header.materialCount * 6);
    m_SRVDescriptorSize = CBaseRenderer::s_TextureHeap.GetDescriptorSize();

    CDescriptorHandle SRVs = m_SRVs;

    CTextureHandle* MatTextures = m_TextureReferences.data();

    for (uint32_t materialIdx = 0; materialIdx < m_Header.materialCount; ++materialIdx)
    {
        const CSimpleMaterial& pMaterial = m_pMaterial[materialIdx];
        ...
        CGraphics::g_Device->CopyDescriptors(1, &SRVs, &DestCount,
