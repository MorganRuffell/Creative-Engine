...
{
    CETBoolean SeparateZPass("Renderer/Separate Z Pass", true);

    bool s_Initialized = false;

    CDescriptorHeap s_TextureHeap;
    CDescriptorHeap s_SamplerHeap;
    CDescriptorHeap s_GUIHeap;

    std::vector<CGraphicsPSO> sm_PSOs;

    CTextureHandle s_RadianceCubeMap;
    CTextureHandle s_IrradianceCubeMap;
    float s_SpecularIBLRange;
    float s_SpecularIBLBias;
    uint32_t g_SSAOFullScreenID;
    uint32_t g_ShadowBufferID;

    CRootSignature m_RootSig;
    CRootSignature m_UIRootSignature;

    CGraphicsPSO m_SkyboxPSO(L"Renderer: Skybox PSO");
    CGraphicsPSO m_DefaultPSO(L"Renderer: Default PSO"); // Not finalized.  Used as a template.

    CDescriptorHandle m_CommonTextures;
}

void CBaseRenderer::Initialize(void)
{
    if (s_Initialized)
        return;

    D3D12_SAMPLER_DESC1 DefaultSamplerDesc;
    DefaultSamplerDesc.MaxAnisotropy = 8;

    D3D12_SAMPLER_DESC1 CubeMapSamplerDesc = DefaultSamplerDesc;
    ...
    CGraphicsPSO DepthOnlyPSO(L"Renderer: Depth Only PSO");
    ...
    CGraphicsPSO CutoutDepthPSO(L"Renderer: Cutout Depth PSO");
    ...
    CGraphicsPSO SkinDepthOnlyPSO = DepthOnlyPSO;
    ...
    CGraphicsPSO SkinCutoutDepthPSO = CutoutDepthPSO;
    ...
    m_DefaultPSO.SetRasterizerState(RasterizerDefaultMsaa);
    ...
    s_GUIHeap.Create(L"ImGUI Sampler Descriptors", D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 2048);
...
void CBaseRenderer::UpdateGlobalDescriptors(void)
{
    ...
    CDescriptorHandle dest = m_CommonTextures + 2 * s_TextureHeap.GetDescriptorSize();
...
void CBaseRenderer::SetIBLTextures(CTextureHandle diffuseIBL, CTextureHandle specularIBL)
{
    ...
    {
        ID3D12Resource2* texRes = const_cast<ID3D12Resource2*>(s_RadianceCubeMap.Get()->GetResource());
...
void CBaseRenderer::SetIBLBias(float LODBias)
{
...
uint8_t CBaseRenderer::GetPSO(uint16_t psoFlags)
{
    using namespace PSOFlags;

    CGraphicsPSO ColorPSO = m_DefaultPSO;
...
void CBaseRenderer::DrawSkybox( CGraphicsContext& gfxContext, const CViewportCamera& Camera, const D3D12_VIEWPORT& viewport, const D3D12_RECT& scissor )
{

    __declspec(align(16)) struct SkyboxVSCB
    {
        CMatrix4 ProjInverse;
        CMatrix3 ViewInverse;
    } skyVSCB;
    ...
    gfxContext.SetDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, s_TextureHeap.GetHeap());
...
void CMeshSerializer::AddMesh( const CStaticMesh& mesh, float distance,
    ...
    const CJoint* skeleton)
{
    ...
    CSkinnedMesh Placeholder;


    SortObject object = { &mesh,&Placeholder,skeleton, meshCBV, materialCBV, bufferPtr };
    m_SortObjects.push_back(object);
}

void CMeshSerializer::Sort()
{
    struct { bool operator()(uint64_t a, uint64_t b) const { return a < b; } } Cmp;
    std::sort(m_SortKeys.begin(), m_SortKeys.end(), Cmp);
}

void CMeshSerializer::RenderMeshes(
    DrawPass pass,
    ...
    GlobalConstants& globals)
{
    ...
    context.SetDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, s_TextureHeap.GetHeap());
    context.SetDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, s_SamplerHeap.GetHeap());
    ...
    for ( ; m_CurrentPass <= pass; m_CurrentPass = (DrawPass)(m_CurrentPass + 1))
    {
        ...
        while (m_CurrentDraw < lastDraw)
        {
            ...
            const CStaticMesh& mesh = *object.mesh;
            const CSkinnedMesh& skinnedMesh = *object.skinnedMeshes;
            ...
            if (skinnedMesh.numJoints > 0)
            {
                ASSERT(object.skeleton != nullptr, "Unspecified joint matrix array");
                context.SetDynamicSRV(kSkinMatrices, sizeof(CJoint) * skinnedMesh.numJoints, object.skeleton + skinnedMesh.startJoint);
            }
