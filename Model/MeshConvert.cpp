...
void OptimizeMesh( CBaseRenderer::Primitive& outPrim, const glTF::Primitive& inPrim, const CMath::CMatrix4& localToObject )
{
    ...
    if (inPrim.indices == nullptr)
    {
        ...
        {
            b32BitIndices = true;
            outPrim.IB = std::make_shared<std::vector<cs_byte>>(4 * indexCount);
        ...
        {
            b32BitIndices = false;
            outPrim.IB = std::make_shared<std::vector<cs_byte>>(2 * indexCount);
            indices = outPrim.IB->data();
    ...
    else
    {
        ...
        case 2: // LINE LOOP
        case 3: // LINE STRIP
            CUtility::Printf("Found unsupported primitive topology\n");
            return;
        case 4: // TRIANGLE LIST
            break;
        case 5: // TODO: Convert TRIANGLE STRIP
        case 6: // TODO: Convert TRIANGLE FAN
            CUtility::Printf("Found an index buffer that needs to be converted to a triangle list\n");
            return;
        }
        ...
        outPrim.IB = std::make_shared<std::vector<cs_byte>>(indexSize * indexCount);
        if (b32BitIndices)
        {
    ...
    const glTF::CSimpleMaterial& material = *inPrim.material;
    ...
    {
        // Local space bounds
        CVector3 sphereCenterLS = (CVector3(*(XMFLOAT3*)inPrim.minPos) + CVector3(*(XMFLOAT3*)inPrim.maxPos)) * 0.5f;
        CScalar maxRadiusLSSq(kZero);

        // Object space bounds
        // (This would be expressed better with an AffineTransform * Vector3)
        CVector3 sphereCenterOS = CVector3(localToObject * CVector4(sphereCenterLS));
        CScalar maxRadiusOSSq(kZero);

        outPrim.m_BBoxLS = CAxisAlignedBox(kZero);
        outPrim.m_BBoxOS = CAxisAlignedBox(kZero);

        for (uint32_t v = 0; v < vertexCount/*maxIndex*/; ++v)
        {
            CVector3 positionLS = CVector3(position[v]);
            maxRadiusLSSq = Max(maxRadiusLSSq, LengthSquare(sphereCenterLS - positionLS));

            outPrim.m_BBoxLS.AddPoint(positionLS);

            CVector3 positionOS = CVector3(localToObject * CVector4(positionLS));
            maxRadiusOSSq = Max(maxRadiusOSSq, LengthSquare(sphereCenterOS - positionOS));

            outPrim.m_BBoxOS.AddPoint(positionOS);
        }

        outPrim.m_BoundsLS = CMath::CSphereCollider(sphereCenterLS, Sqrt(maxRadiusLSSq));
        outPrim.m_BoundsOS = CMath::CSphereCollider(sphereCenterOS, Sqrt(maxRadiusOSSq));
    ...
    outPrim.VB = std::make_shared<std::vector<cs_byte>>(stride * vertexCount);
    ...
    outPrim.DepthVB = std::make_shared<std::vector<cs_byte>>(depthStride * vertexCount);
