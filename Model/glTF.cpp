...
inline uint32_t floatToHalf( float f )
{
    ...
    x.f = CMath::Clamp(f, 0.0f, 1.0f) * kF32toF16; 
    return x.u >> 13;
}
...
void glTF::Asset::ProcessMaterials( json& materials )
{
    ...
    for (json::iterator it = materials.begin(); it != materials.end(); ++it)
    {
        ...
        if (thisMaterial.find("pbrMetallicRoughness") != thisMaterial.end())
        {
            ...
            for (uint32_t i = 0; i < CSimpleMaterial::kNumTextures; ++i)
            ...
            if (metallicRoughness.find("baseColorTexture") != metallicRoughness.end())
                material.baseColorUV = ReadTextureInfo(metallicRoughness.at("baseColorTexture"),
                    material.textures[CSimpleMaterial::kBaseColor]);
            ...
            if (metallicRoughness.find("metallicRoughnessTexture") != metallicRoughness.end())
                material.metallicRoughnessUV = ReadTextureInfo(metallicRoughness.at("metallicRoughnessTexture"),
                    material.textures[CSimpleMaterial::kMetallicRoughness]);
        }
...
void glTF::Asset::ProcessBuffers( json& buffers, CSmartByteVector chunk1bin )
{
    ...
    for (json::iterator it = buffers.begin(); it != buffers.end(); ++it)
    {
        ...
        if (thisBuffer.find("uri") != thisBuffer.end())
        {
            ...
            CSmartByteVector ba = ReadFileSync(filepath);
...
void glTF::Asset::Parse(const std::wstring& filepath)
{
    ...
    CSmartByteVector gltfFile;
    CSmartByteVector chunk1Bin;

    std::wstring fileExt = CUtility::ToLower(CUtility::GetFileExtension(filepath));
    ...
    if (fileExt == L"glb")
    {
        ...
        {
            CUtility::Printf("Error:  Invalid glTF binary format\n");
            return;
        }
        if (header.version != 2)
        {
            CUtility::Printf("Error:  Only glTF 2.0 is supported\n");
            return;
        }
        ...
        {
            CUtility::Printf("Error: Expected chunk0 to contain JSON\n");
            return;
        }
        gltfFile = make_shared<vector<cs_byte>>( chunk0Length + 1 );
        ...
        {
            CUtility::Printf("Error: Expected chunk1 to contain BIN\n");
            return;
       }

        chunk1Bin = make_shared<vector<cs_byte>>(chunk1Length);
    ...
    else 
    {
        ...
        chunk1Bin = make_shared<vector<cs_byte>>(0);
    }
    ...
    m_basePath = CUtility::GetBasePath(filepath);
