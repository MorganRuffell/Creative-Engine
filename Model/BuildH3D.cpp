...
bool CMesh::BuildModel(ModelData& model, const std::wstring& basePath) const
{
    model.m_SceneGraph.resize(1);

    CGraphNode& node = model.m_SceneGraph[0];
    node.xform = CMatrix4(kIdentity);
    node.rotation = CQuarternion(kIdentity);
    ...
    {
        const CSimpleMaterial& material = GetMaterial(i);
        ...
        const wstring baseColorFullPath = basePath + CUtility::UTF8ToWideString(baseColorPath);
        const wstring metallicRoughnessFullPath = basePath + CUtility::UTF8ToWideString(metallicRoughnessPath);
        const wstring emissiveFullPath = basePath + CUtility::UTF8ToWideString(emissivePath);
        const wstring normalFullPath = basePath + CUtility::UTF8ToWideString(normalPath);
        ...
        const string lowerCaseName = CUtility::ToLower(baseColorPath);
    ...
    {
        std::wstring fullPath = basePath + CUtility::UTF8ToWideString(model.m_TextureNames[ti]);
    ...
    {
        const CBaseMesh& mesh = GetMesh(i);
        ...
        CMath::CVector3 Min = mesh.boundingBox.GetMin();
        CMath::CVector3 Max = mesh.boundingBox.GetMax();
        ...
        CBaseRenderer::CompileMesh(model.m_Meshes, model.m_GeometryData, gltfMesh, 0, CMatrix4(kIdentity), sphereOS, boxOS); 
