...
bool CStaticMesh::SM_LoadData()
{
	...
	//HRESULT Resultb = GraphicsCard->CreateShaderResourceView(SM_Resource, &ShaderResourceView, &SM_Shaders->SM_ShaderResourceView);
	//if (FAILED(Resultb))
	//	return false;

	//HRESULT Resulta = GraphicsCard->CreateBuffer(&VertexDESC,&resourceData, &SM_Shaders->SM_GraphicsBuffer);
	//if (FAILED(Resulta))
	//	return false;
